...
const store = new Vuex.Store({
  ...
  getters: {
    juegosConStock: state => {
      return state.juegos.filter(juego => parseInt(juego.stock) > 0)
    },
    juegoDisponiblePorId: (state, getters) => (codigo) => {
      return getters.juegosConStock.filter(juego => juego.codigo == codigo)
    },
    totalStock: state => {
      let stock = 0
      for(let unidad of state.juegos){
        stock += unidad.stock
      }
      return stock
    },
